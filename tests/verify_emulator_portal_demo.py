"""Offline portal story acceptance; rebuild first. Phase 5 settings includes it."""
import json
import sys
...
path=ROOT/'docs/ui-emulator/portal-demo-verification.json'
report={'status':'running','recorded_at':datetime.now(timezone.utc).isoformat(),'checks':[],
        'limitations':['Synthetic clients and a fixed demo password only; no server or credential input',
                       'Portal browser cases serve unchanged local Wasm bytes via Playwright; HTTP delivery is tested separately']}
try:
    fingerprint=artifact_fingerprint();report.update(fingerprint)
    for command in [['node','--test','tests/test_emulator_portal_demo.mjs','tests/test_emulator_attacks.mjs','tests/test_emulator_bridge.mjs'],
                    [sys.executable,'tests/test_emulator_portal_demo.py'],
